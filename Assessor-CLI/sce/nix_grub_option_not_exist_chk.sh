...
if [ -f "$grubfile" ]; then
	if [ "$gfsn" = "grub.conf" ]; then
		! grep -P "^\s*kernel" "$grubfile" | grep -Pq "\b$XCCDF_VALUE_REGEX\b" && passing=true
		output="$(grep -P '^\h*kernel.*$' "$grubfile" | grep -P "\b$XCCDF_VALUE_REGEX\b")"
	elif [ "$gfsn" = "grub.cfg" ]; then
		! grep -P "^\h*linux.*$" "$grubfile" | grep -Pq "\b$XCCDF_VALUE_REGEX\b" && passing=true
		output="$(grep -P "^\h*linux.*$" "$grubfile" | grep -P "\b$XCCDF_VALUE_REGEX\b")"
	elif [ "$gfsn" = "grubenv" ]; then
		! grep -P "^\h*kernelopts=" "$grubfile" | grep -Pq "\b$XCCDF_VALUE_REGEX\b" && passing=true
...
if [ "$passing" = true ] ; then
	...
	echo "FAILED:"
	echo "\"$grubfile\" contains: \"$XCCDF_VALUE_REGEX\""
	echo $output
